...
public final class ArrayUtil {
  ...
  */
   ...
   * NumberFormatException if the string does not represent an int quantity.
   *
  ...
  */
   ...
   * LICENSE.txt), but has now been substantially changed based on discussions from java-dev
   * thread with subject "Dynamic array reallocation algorithms", started on Jan 12 2010.
   *
  ...
  public static short[] shrink(short[] array, int targetSize) {
    assert
        targetSize >= 0 :
        "size must be positive (got " + targetSize + "): likely integer overflow?";
  ...
  public static int[][] shrink(int[][] array, int targetSize) {
    ...
    int
        newSize =
        getShrinkSize(array.length, targetSize, RamUsageEstimator.NUM_BYTES_OBJECT_REF);
    if (newSize != array.length) {
      int[][] newArray = new int[newSize][];
      System.arraycopy(array, 0, newArray, 0, newSize);
      return newArray;
   ...
   */
  public static boolean equals(char[] left, int offsetLeft, char[] right, int offsetRight,
                               int length) {
  ...
  */
   ...
   * @see java.util.Arrays#equals(char[], char[])
   */
  public static boolean equals(int[] left, int offsetLeft, int[] right, int offsetRight,
                               int length) {
    if ((offsetLeft + length <= left.length) && (offsetRight + length <= right.length)) {
      for (int i = 0; i < length; i++) {
        if (left[offsetLeft + i] != right[offsetRight + i]) {
