...
/**
 * An iterator to iterate over set bits in an OpenBitSet. This is faster than nextSetBit() for
 * iterating over the complete set of bits, especially when the density of the bits set is high.
 */
public class OpenBitSetIterator extends DocIdSetIterator {
  ...
  /**
   * *** the python code that generated bitlist def bits2int(val): arr=0 for shift in
  ...
  /**
   * *** alternate shift implementations // 32 bit shifts, but a long shift needed at the end
   * private void shift2() { int y = (int)word; if (y==0) {wordShift +=32; y = (int)(word >>>32);
   * } if ((y & 0x0000FFFF) == 0) { wordShift +=16; y>>>=16; } if ((y & 0x000000FF) == 0) {
