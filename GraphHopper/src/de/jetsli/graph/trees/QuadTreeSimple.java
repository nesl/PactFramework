...
public class QuadTreeSimple<T> implements QuadTree<T> {
  ...
  public void add(double lat, double lon, T value) {
    ...
    throw new UnsupportedOperationException(
        "Cannot put element? Too many entries per area? Try increasing entries per leaf! "
  ...
  private void addData(long spatialKey, T value, QTNode<T> current, QTNode<T> previousBranch,
    ...
    throw new IllegalStateException(
        "Cannot happen? datanode:" + dataNode + " new entry:" + spatialKey + "->" + value);
  }
  ...
  public int remove(double lat, double lon) {
    ...
    getNeighbours(BBox.createEarthMax(), new BBox(lon - err, lon + err, lat - err, lat + err), root,
                  worker);
  ...
  @Override
  public Collection<CoordTrig<T>> getNodesFromValue(final double lat, final double lon,
                                                    final T value) {
    if (root == null) {
