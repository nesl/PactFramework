...
import static de.jetsli.graph.util.GraphUtility.count;
...
public class MMapGraph implements SaveableGraph {
  ...
  protected boolean ensureNodesCapacity(int newNumberOfNodes) throws IOException {
    ...
    if (nodes != null) {
      ...
      newNumberOfNodes =
          (int) Math.max(newNumberOfNodes, increaseFactor * nodes.capacity() / bytesNode);
  ...
  public void edge(int a, int b, double distance, int flags) {
    if (distance <= 0) {
      throw new UnsupportedOperationException("negative or zero distances are not supported:"
                                              + a + " -> " + b + ": " + distance
                                              + ", bothDirections:" + BitUtil
          .toBitString(flags, 16));
    }
  ...
  public EdgeIdIterator getOutgoing(int index) {
    ...
    return new EdgesIteratorableFlags(
        getBytesFromNodes(index * bytesNode + bytesNodeCore, bytesEdges), (byte) 1);
  }
  ...
  public EdgeIdIterator getIncoming(int index) {
    ...
    return new EdgesIteratorableFlags(
        getBytesFromNodes(index * bytesNode + bytesNodeCore, bytesEdges), (byte) 2);
  }
  ...
  protected int getNextFreeEdgeBlock() {
    ...
    if (nextEdgePos > edges.capacity()) {
      throw new IllegalStateException(
          "You need to ensure capacity of edges before creating new ones");
    }
    return tmp;
  }
