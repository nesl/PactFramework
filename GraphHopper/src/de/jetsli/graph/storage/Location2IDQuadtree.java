...
public class Location2IDQuadtree implements Location2IDIndex {
  ...
  private void initAlgo(int bits) {
    double minLon = Double.MAX_VALUE, maxLon = Double.MIN_VALUE, minLat = Double.MAX_VALUE,
        maxLat =
            Double.MIN_VALUE;
    int locs = g.getNodes();
    ...
    maxNormRasterWidthKm =
        calc.normalizeDist(Math.max(calc.calcDistKm(minLat, minLon, minLat, maxLon),
                                    calc.calcDistKm(minLat, minLon, maxLat, minLon)) / Math
            .sqrt(size));
          ...
          if (tmpKey < 0 || tmpKey >= size) {
            if (keyOffset >= size) {
              throw new IllegalStateException(
                  "couldn't find any node!? " + tmpKey + " " + keyOffset + " " + size);
            }
...
//            });
      ...
      if (mainKey < 0 || mainKey >= size) {
        throw new IllegalStateException(
            "Problem with mainKey:" + mainKey + " " + mainCoord + " size:" + size);
      }
