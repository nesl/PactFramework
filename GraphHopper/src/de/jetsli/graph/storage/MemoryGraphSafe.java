 ...
 * storage if existing. It allows duplicate edges if flags (highway and bothDir flags) are
 * identical
 *
 * @author Peter Karich
 */
public class MemoryGraphSafe implements SaveableGraph {
  ...
  int
      LEN_EDGE =
      LEN_NODEA_ID + LEN_NODEB_ID + LEN_LINKA + LEN_LINKB + LEN_FLAGS + LEN_DIST;
...
//            replacingDelete(deleted);
  ...
  void inPlaceDelete(int deleted) {
    ...
    for (int delNode = deletedNodes.next(0); delNode >= 0;
         delNode = deletedNodes.next(delNode + 1)) {
    ...
    for (int toUpdateNode = toUpdatedSet.next(0); toUpdateNode >= 0;
         toUpdateNode = toUpdatedSet.next(toUpdateNode + 1)) {
      ...
      while (movedEdgeIter.next()) {
        if (deletedNodes.contains(movedEdgeIter.nodeId())) {
          throw new IllegalStateException(
              "shouldn't happen the edge to the node " + movedEdgeIter.nodeId()
              + " should be already deleted. " + oldI);
        }
  ...
  void replacingDelete(int deleted) {
    ...
    for (int oldNodeId = 0; oldNodeId < locs; oldNodeId++) {
      ...
      while (iter.next()) {
        ...
        inMemGraph
            .internalEdgeAdd(newNodeId, old2NewMap[iter.nodeId()], iter.distance(), iter.flags());
      }
      newNodeId++;
    }
